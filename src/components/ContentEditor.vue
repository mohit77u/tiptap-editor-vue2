<template>
    <div v-if="editor">
        ...
        <div class="fixed-menu absolute top-0 left-0 w-full flex flex-wrap gap-2 items-center px-2 h-[38px] leading-[20px]" v-if="editableEditor && editorVisible">
            ...
            <button class="single-block icon-btn group relative px-2 py-1.5 hover:bg-blue-100/80 rounded" :class="{ 'bg-blue-100/80': menu.isActiveTest(editor), 'pointer-none': (menu.label == 'Undo' ? !editor.can().chain().focus().undo().run() : (menu.label == 'Redo') ? !editor.can().chain().focus().redo().run() : false) }" v-for="(menu, index) in inline_menus" :key="index" @click="menu.label != 'Link' ? menu.command(editor) : setLink()" :disabled="(menu.label == 'Undo' ? !editor.can().chain().focus().undo().run() : (menu.label == 'Redo') ? !editor.can().chain().focus().redo().run() : false) ">
                <div class="icon" :class="{ 'text-lime-500': menu.isActiveTest(editor), 'text-slate-800': !menu.isActiveTest(editor), 'opacity-20': (menu.label == 'Undo' ? !editor.can().chain().focus().undo().run() : (menu.label == 'Redo') ? !editor.can().chain().focus().redo().run() : false) }" v-html="menu.icon"></div>
            ...
            <div class="font-size relative">
                <div class="flex items-center border border-gray-200 rounded">
                    <button class="text-[26px] text-slate-800 w-7 h-8 bg-blue-100/80" @click="decreaseFontSize">-</button>
                    <input type="text" :value="fontSize" @change="applyFontSize($event, '')" @focus="showFontOptions = true" @blur="showFontOptions = false" class="w-[40px] h-8 px-1 text-[16px] border-x border-gray-200 text-center focus:outline-none font-normal">
                    <button class="text-[16px] text-slate-800 w-7 h-8 bg-blue-100/80" @click="increaseFontSize">+</button>
                </div>
            </div>
